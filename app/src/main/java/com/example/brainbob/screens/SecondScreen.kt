...
@Composable
fun rowList(){
    val tagList= listOf(Tags("Brainstorm"), Tags("Books"),
...
fun tagCard(tag: String){
   ...
   , shape = RoundedCornerShape(40.dp)) {
        Text(text = tag,style = typography.subtitle2,
...
Row(verticalAlignment = Alignment.CenterVertically,
    ...
    Column(modifier = Modifier.padding(8.dp),
      ...
      Text(text = "Vocabulary", style = typography.h3,
          modifier = Modifier.padding(top = 8.dp))
      Text(text = "Listen 20 new words", style= typography.body2,modifier = Modifier.padding(8.dp))
...
@Composable
fun Recommended(){
    var RecList= listOf(Recommendation("Chatting","5 minutes", R.drawable.chat)
...
LazyColumn(Modifier.fillMaxWidth()){
    item {
        Row() {
            Text(text ="Recommended", style= typography.h1,
                modifier = Modifier.padding(top=16.dp))
        }
    }

       items(RecList){Recommendation ->
           RecommendationCard(Recommendation.type, Recommendation.min
           , Recommendation.icon)
       }
   }
